...
import { createDefect, fetchAllDefects } from "../../actions";
import DefectsForm from "./DefectsForm";
import _ from 'lodash';

class DefectsCreate extends React.Component{

    state = { newID: 'Loading', newBugID: 'Loading'};

    newDate = new Date().toDateString();

    renderState = async () => {
        await this.setState({newID: parseInt(this.props.lastDefect.id) + 1});
        this.setState({newBugID:`DEF-${this.state.newID}`});
    };

    componentDidMount() {
        if (!this.props.lastDefect) {

            const fetchAllData = async () => {
                await this.props.fetchAllDefects();
                this.renderState();
            };
    ...
    onSubmit = (formValues) => {
        ...
        console.log({...formValues, id: this.state.newID, bugId: this.state.newBugID, created: this.newDate});
    };
    ...
    render() {
        ...
        return (
            <div>
                <DefectsForm bugId={this.state.newBugID} onSubmit={this.onSubmit} />
            </div>
        );
    }
}
...
const mapStateToProps = (state) => {
    ...
    return { lastDefect: state.defects[_.size(state.defects)] }
};

export default connect(mapStateToProps, { createDefect, fetchAllDefects })(DefectsCreate);
